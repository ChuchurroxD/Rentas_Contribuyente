...
{
    public partial class Frm_Predio_MasivoActivo : DockContent
    {
        ...
        private Int32 periodoviejo;
        private string usser = GlobalesV1.Global_str_Usuario;
        public Frm_Predio_MasivoActivo()
        {
            InitializeComponent();
            periodoActivo = PeriodoDataService.GetPeriodoActivo();
            periodoviejo = periodoActivo - 1;
            lblPeriodo.Text = periodoActivo.ToString();
        }
        private void btnVerificar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    listapm = ParametroMesDataService.listarerrores(periodoActivo, 7, 1, "0043");
                    listaArbitrioDesc = Pred_PrredioArbitrioDataService.verificarArbitriomASIVO(periodoviejo, periodoActivo);
                    //listaArbitrioCant = Pred_PrredioArbitrioDataService.cantidadArbitriomASIVO(periodoviejo);
                }
                else if (rbtPredio.Checked)
                {
                    listapm = ParametroMesDataService.listarerrores(periodoActivo, 4, 1, "0008");
                    listaarancel = PredioDataService.listarerrores(periodoActivo);
                    lista = DepreciacionDataService.listarErroresDepreciacion(periodoActivo);
                }
        ...
        private void btnGenerarCalculoMasivo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (rbtPredio.Checked)
                {
                    ...
                    //if (Proc_CuentaCorritenteDataService.VerificarExisteCtaCTe(periodoActivo, tipo) == 0)
                    //{
                    if (MessageBox.Show("Desea Generar cta cte?", Globales.Global_MessageBox, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        PredioDataService.COpiarPredio_Piso_predContribuyente(periodoviejo, periodoActivo, usser);
                        PredioDataService.GenerarPredioMasivo(periodoviejo, periodoActivo, usser);
                ...
                else
                {
                    ...
                    if (MessageBox.Show("Desea Generar cta cte?", Globales.Global_MessageBox, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        Pred_PrredioArbitrioDataService.copiarArbitriomASIVO(periodoviejo, periodoActivo, usser);
                        Pred_PrredioArbitrioDataService.GenerarCalculoMasivo(usser, periodoActivo);
                        MessageBox.Show("Se generó cuenta corriente", Globales.Global_MessageBox);
                    }
                    //}
                    //else
