...
begin
	...
	set @per_pass = CONVERT(nvarchar(300), @persona_id)

	insert into usuario (per_codigo,areas_codarea, estado, per_login, per_pass, nombre, per_codContributente, fecha_creacion) 
	values(@usuario_id, '06', 1, @persona_id, ENCRYPTBYPASSPHRASE('SGR_MOCHE', @per_pass), @razon_social, @persona_id, getdate())
	fetch cursorUsuario
	into @persona_id, @razon_social
end
close cursorUsuario
